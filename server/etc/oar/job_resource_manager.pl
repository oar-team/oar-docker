...
# Be careful with the physical_package_id. Is it corresponding to the memory banc?
        if (system( 'oardodo mkdir -p /dev/cpuset/'.$Cpuset_path_job.' && '.
                    ...
                    '/bin/echo 0 | cat > /dev/cpuset/'.$Cpuset_path_job.'/cpuset.cpu_exclusive && '.
                    'cat /dev/cpuset/cpuset.mems > /dev/cpuset/'.$Cpuset_path_job.'/cpuset.mems && '.
                    '/bin/echo '.join(",",@Cpuset_cpus).' | cat > /dev/cpuset/'.$Cpuset_path_job.'/cpuset.cpus'
                  )){
